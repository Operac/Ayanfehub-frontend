...
export default function AdminProductApprovalTab() {
  ...
  const fetchPendingProducts = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/admin/products/pending?page=${page}&limit=${limit}`,
        { credentials: 'include' }
      );
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      showToast('Product rejected. Vendor will be notified.', 'success');
  ...
  if (loading && products.length === 0) {
    return <div className="text-center py-8 text-muted">Loading pending products...</div>;
  }
